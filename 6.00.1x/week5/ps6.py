...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        self.shift_dict = dict(zip(fullKeys, fullValues))
    ...
    def apply_shift(self, shift):
        ...
        for character in self.message_text:
            if character not in self.build_shift_dict(shift).keys():
                msgShifted.append(character)
                continue
            else:
                msgShifted.append(self.build_shift_dict(shift)[character])
        return ''.join(msgShifted)
...
class PlaintextMessage(Message):
    ...
    def change_shift(self, shift):
        ...
        Returns: nothing
        '''
        assert(0 <= shift and shift < 26), 'Shift must be an int between 0 and 26'
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(1, 27):
            valid_words = 0
            messageDecrypted = Message.apply_shift(self, 26 - shift)
            wordList = messageDecrypted.split(' ')
            
            for word in wordList:
                if is_word(self.valid_words, word):
                    valid_words += 1
            
            if valid_words > bestTry:
                bestTry = valid_words
                bestShift = (26 - shift)
                bestMessage = messageDecrypted
        
        return (bestShift, bestMessage)

def decrypt_story():
    '''
    Function that load a file with an encrypted story string and returns the 
    appropriate shift value and unencrypted story string.
    '''
    encrypted = get_story_string()
    cipher = CiphertextMessage(encrypted)
